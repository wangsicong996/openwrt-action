...
# Modified Time: 2026-09-02 07:07:00 UTC
#########################################################################
...
stat_cfg="feeds/luci/applications/luci-app-statistics/root/etc/config/luci_statistics"
if [ -f "$stat_cfg" ]; then
...
if [ -f feeds/packages/net/cshark/Makefile ]; then
...
if [ -f feeds/CustomPkgs/net/ipv6-helper/Makefile ]; then
...
if [ -f feeds/routing/hnetd/Makefile ]; then
...
# set v2raya depends on v2ray-core (package may be absent)
if [ -f feeds/packages/net/v2raya/Makefile ]; then
...
if [ -f "${RRDTOOL_PATH}/Makefile" ]; then
